"""CLI: uv run python -m html_pipeline <subcommand> [...]"""
...
# allow `python -m html_pipeline` to run from inside data/corpus/
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from html_pipeline import config as cfg_mod  # noqa: E402
from html_pipeline import pipeline as pl  # noqa: E402
...
def cmd_list(args) -> None:
    ...
    if not ids:
        print("No HTML-source guidelines found in guidelines/")
        return
    print(f"\nHTML-source guidelines  ({cfg_mod.GUIDELINES_DIR}):\n")
    for gid in ids:
        try:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="html_pipeline",
        description="Subsection-level HTML extraction pipeline.",
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    p_list = sub.add_parser("list", help="List HTML-source guidelines")
