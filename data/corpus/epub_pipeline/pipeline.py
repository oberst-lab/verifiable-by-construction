...
logger = setup_logging("epub_pipeline")

STAGE_ORDER = ["extract", "describe_visuals", "summarize"]
STATE_FILENAME = ".epub_pipeline_state.json"
...
def run(
    ...
    *,
    ...
    """Run a sequence of stages for one guideline."""
    force_stages = force_stages or set()
    stages = stages or STAGE_ORDER

    state = load_state(cfg.guideline_dir)
    logger.info("🏥 epub_pipeline: %s → %s", cfg.guideline_id, cfg.guideline_dir)
    if dry_run:
...
def status(cfg: GuidelineConfig) -> dict:
    """Return a brief status dict for CLI display."""
    state = load_state(cfg.guideline_dir)
